...
const RecipeTimeOutput: React.FC<Props> = ({ preparationTime, pressTime, isSmall }) => {
    return (
        <div className={`flex w-full ${isSmall ? '' : 'p-3'} justify-center text-orange-950`}>
            <div className="flex justify-center items-center w-1/2">
                <svg
                    ...
                    strokeWidth="1"
                    stroke="currentColor"
                    className={`w-${isSmall ? '5' : '10'} h-${isSmall ? '5' : '10'}`}
                >
                    <path
                        strokeLinecap="round"
                ...
                <div className="mx-3">
                    <p className={`text-${isSmall ? 'xs' : 'sm'}`}>準備</p>
                    <p className={`text-${isSmall ? 'sm' : 'lg'} font-medium`}>{preparationTime} 分</p>
                </div>
            </div>
            <div className="flex justify-center items-center w-1/2">
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`w-${isSmall ? '5' : '10'} h-${isSmall ? '5' : '10'} fill-current`}
                ...
                <div className="mx-3">
                    <p className={`text-${isSmall ? 'xs' : 'sm'}`}>加圧</p>
                    <p className={`text-${isSmall ? 'sm' : 'lg'} font-medium`}>{pressTime} 分</p>
                </div>
            </div>
        </div>
    );
